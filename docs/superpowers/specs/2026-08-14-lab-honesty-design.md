...
- Prefer real chat models. `speed=quality` (default) ranks by tier; `speed=fast` ranks small / flash / lite first. Ping **up to 3** per host **in parallel**. Host row includes `models[]`; `model_id` is the first `up` id.
